    ...
    def start(self):
        ...
        for idx in pbar:
            ...
            if imagepath.split('.')[-1] not in ['jpg', 'jpeg', 'png']:
                print(imagepath, 'is not an image file, please check the imagepath')
                continue
            ...
            if cmd_args.outputdir:
                base_path = os.path.join(cmd_args.outputdir, imagepath.split('/')[-1].split('.')[0])
                print("base path: ", base_path)
                os.makedirs(base_path, exist_ok=True)
                cv2.imwrite(os.path.join(base_path, 'image_mask.png'), image)
                cv2.imwrite(os.path.join(base_path, 'color_mask.png'), seg_mask)
                cv2.imwrite(os.path.join(base_path, 'mask.png'), seg_pred.astype(np.uint8))
            else:
